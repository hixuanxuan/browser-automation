...
async function getTabWsUrl(cdpBase, tabIdentifier) {
    ...
    if (!tab) {
        throw new Error(`Tab "${tabIdentifier}" not found at ${cdpBase}`);
    }
...
const PALETTE = {
    red: {
        ...
        labelBg: 'rgba(180,20,20,0.93)',
        text: '#ffffff',
    },
    green: {
        stroke: 'rgba(21,128,61,0.95)',
        tint: 'rgba(21,128,61,0.12)',
        labelBg: 'rgba(14,100,47,0.93)',
        text: '#ffffff',
    },
};
...
function renderWidthBracket(rx, ry, rw, rh, c, label) {
    ...
    const parts = [
        ...
        `<line x1="${rx + rw}" y1="${y - DIM_TICK}" x2="${rx + rw}" y2="${
            y + DIM_TICK
        }" stroke="${c.stroke}" stroke-width="2"/>`,
        `<rect x="${cx - tw / 2}" y="${
            y - DIM_TICK - th - 2
        }" width="${tw}" height="${th}" rx="3" fill="${c.labelBg}"/>`,
        `<text x="${cx}" y="${
            y - DIM_TICK - 6
        }" text-anchor="middle" font-family="ui-monospace,Menlo,monospace" font-size="${DIM_FONT}" fill="${c.text}">${
            esc(label)
        }</text>`,
    ];
...
function renderHeightBracket(rx, ry, rw, rh, c, label) {
    ...
    const parts = [
        ...
        `<line x1="${x - DIM_TICK}" y1="${ry + rh}" x2="${x + DIM_TICK}" y2="${
            ry + rh
        }" stroke="${c.stroke}" stroke-width="2"/>`,
        `<rect x="${x + DIM_TICK + 4}" y="${cy - th / 2}" width="${tw}" height="${th}" rx="3" fill="${c.labelBg}"/>`,
        `<text x="${x + DIM_TICK + 10}" y="${
            cy + DIM_FONT / 2 - 2
        }" font-family="ui-monospace,Menlo,monospace" font-size="${DIM_FONT}" fill="${c.text}">${esc(label)}</text>`,
    ];
    ...
    for (const ann of annotations) {
        ...
        if (type === 'width') {
            ...
            if (ann.highlight === 'fill') {
                ...
                parts.push(
                    `<rect x="${rx}" y="${ry}" width="${rw}" height="${rh}" fill="none" stroke="${c.stroke}" stroke-width="2"/>`
                );
            }
            ...
            parts.push(
                `<line x1="${elCx}" y1="${elCy}" x2="${nearX}" y2="${nearY}" stroke="${c.stroke}" stroke-width="1.5" stroke-dasharray="4 3" opacity="0.75"/>`
            );
            parts.push(
                `<rect x="${box.x}" y="${box.y}" width="${box.w}" height="${box.h}" rx="4" fill="${c.labelBg}"/>`
            );
            labelLines.forEach((line, i) => {
                parts.push(
                    `<text x="${box.x + LABEL_PAD}" y="${
                        box.y + LABEL_PAD + (i + 1) * LINE_HEIGHT - 4
                    }" font-family="ui-monospace,Menlo,monospace" font-size="${FONT_SIZE}" fill="${c.text}">${
                        esc(line)
                    }</text>`
                );
            });
