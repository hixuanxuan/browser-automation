...
if (!figmaUrl || !token) {
  console.error('Usage: export-image.mjs --figma-url <url> --token <figma_token> [--scale <n>] [--format png|jpg|svg|pdf] [--output <path>]');
...
const imagesUrl = `https://api.figma.com/v1/images/${fileKey}?ids=${encodeURIComponent(apiNodeId)}&format=${format}&scale=${scale}`;
