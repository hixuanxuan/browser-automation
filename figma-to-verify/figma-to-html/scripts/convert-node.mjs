...
if (!figmaUrl || !token) {
    console.error(
        'Usage: convert-node.mjs --figma-url <url> --token <figma_token> [--username <name>] [--output <dir>]'
    );
...
async function fetchJson(url, options = {}) {
    ...
    if (!response.ok) {
        ...
        throw new Error(
            `Request failed (${response.status} ${response.statusText}) for ${url}${text ? `: ${text}` : ''}`
        );
    }
...
const nodeInfo = await resolveNodeInfo(fileKey, apiNodeId, nodeId).catch(() => ({
    name: nodeId,
    width: null,
    height: null,
}));
